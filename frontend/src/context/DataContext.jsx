...
export function DataProvider({ children }) {
  const [data, setData] = useState(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        // If saved state claims user is logged in but no real auth token
        // exists (e.g. it was cleared, or the browser cleared storage),
        // force isLoggedIn = false so the user goes to /login on refresh.
        const storedToken = localStorage.getItem('mindease_auth_token');
        if (parsed?.isLoggedIn && (!storedToken || storedToken.trim() === '')) {
          parsed.isLoggedIn = false;
          parsed.profile = { ...initialDefaultData.profile, name: '', email: '', avatar: '', bio: '' };
        }
  ...
  const [dailyCheckins, setDailyCheckins] = useState(() => {
    ...
    try {
      const hints = [
        ...
        data?.profile?.email ? data.profile.email.trim().toLowerCase() : null
      ].filter(Boolean);

      for (const hint of hints) {
        const cleanKey = String(hint).replace(/[^a-zA-Z0-9_@.-]/g, '_');
        const raw = localStorage.getItem(`mindease_daily_checkins_${cleanKey}`);
        if (raw) {
  ...
  useEffect(() => {
    // Only refresh check-in data when the user IS logged in.
    // When isLoggedIn becomes false (logout), data.profile is already blank,
    // so refreshDailyCheckins(null) would read the 'guest' cache (empty) and
    // wipe dailyCheckins → dailyStreak to 0. Skip it on logout.
  ...
  const refreshLiveData = useCallback(async () => {
    ...
    // Only update the email field from the authenticated server response.
    // NEVER fall back to a hard-coded demo email — that would overwrite the
    // real user's profile with the placeholder account on any API failure.
    const serverEmail = (res?.user?.email && res.user.email.includes('@'))
      ? res.user.email.trim().toLowerCase()
      : null;
    const userEmail = serverEmail || cleanEmail || '';
    ...
    setData((prev) => {
      ...
      return {
        ...prev,
        // Only update profile fields that the server actually returned.
        // If res is null (API failed), keep every field exactly as it was.
        profile: {
          ...prev.profile,
          ...(res?.user?.name ? { name: res.user.name } : {}),
          ...(userEmail ? { email: userEmail } : {}),
          avatar: finalAvatar || prev.profile?.avatar,
          ...(res?.user?.bio !== undefined ? { bio: res.user.bio } : {}),
          ...(res ? { streak: res?.daily_checkin_streak ?? res?.streak ?? prev.profile?.streak ?? 0 } : {}),
          ...(res?.total_sessions !== undefined ? { totalSessions: res.total_sessions } : {}),
          ...(res?.completed_programs_count !== undefined ? { completedProgramsCount: res.completed_programs_count } : {}),
          ...(res?.saved_resource_ids ? { savedResourceIds: res.saved_resource_ids } : {}),
          ...(res?.assessment_history ? { assessmentHistory: res.assessment_history } : {})
        },
  ...
  // Runs once on mount. Checks whether the stored JWT is still valid by
  // calling GET /api/auth/me.
  useEffect(() => {
    const validateTokenOnMount = async () => {
      const token = localStorage.getItem('mindease_auth_token');

      // No token at all — mark as logged out with clean empty profile
      if (!token || token.trim() === '') {
        setData((prev) => ({
          ...prev,
          isLoggedIn: false,
          profile: { ...initialDefaultData.profile, name: '', email: '', avatar: '', bio: '' }
        }));
      ...
      try {
        ...
        if (response.status === 401 || response.status === 403) {
          console.warn('Stored token is invalid/expired. Re-authentication required.');
          ...
          const serverStreak = meData?.daily_checkin_streak;

          if (serverUser) {
            const cleanEmail = (serverUser.email && serverUser.email.includes('@')) ? serverUser.email.trim().toLowerCase() : '';
            const savedAvatar = cleanEmail ? localStorage.getItem(`mindease_user_avatar_${cleanEmail}`) : null;
            const finalAvatar = savedAvatar || serverUser.avatar_url || '';
            ...
            setData((prev) => ({
              ...
              profile: {
                ...prev.profile,
                id:     serverUser.id     || prev.profile?.id,
                name:   serverUser.name   || prev.profile?.name,
                email:  cleanEmail        || prev.profile?.email,
                avatar: finalAvatar       || prev.profile?.avatar,
                bio:    serverUser.bio !== undefined ? serverUser.bio : (prev.profile?.bio || ''),
                streak: (typeof serverStreak === 'number') ? serverStreak : (prev.profile?.streak || 0)
              },
              settings: {
                ...prev.settings,
                ...(settings || {})
        ...
        console.warn('Token validation notice — backend unreachable:', networkErr.message);
      } finally {
        setIsAuthLoading(false);
      }
    };
  ...
  const callDirectOpenRouter = async (messagesHistory, userText) => {
    ...
    if (!OPENROUTER_API_KEY) {
      console.warn('VITE_OPENROUTER_API_KEY not set – skipping direct OpenRouter call');
      return null;
    }
...
CRITICAL REQUIREMENT: Always maintain exact context of what the user has told you earlier in this session (such as their name, job, feelings, goals, or past messages). Directly refer to their previous details when relevant. Keep normal responses short (1-3 sentences), warm, and natural.`;
    ...
    // Ensure the current user message is the final turn
    ...
    for (const model of modelsToTry) {
      try {
        ...
        if (response.ok) {
          ...
          if (resData?.error) {
            console.warn(`OpenRouter API-level error for model ${model}:`, resData.error);
            continue;
          }
          const reply = resData?.choices?.[0]?.message?.content?.trim();
          if (reply) {
            console.info(`OpenRouter direct reply via model: ${model}`);
            return reply;
          }
        } else {
          const errBody = await response.json().catch(() => ({}));
          console.warn(`OpenRouter HTTP ${response.status} for model ${model}:`, errBody?.error || response.statusText);
        }
      } catch (err) {
        if (err.name === 'AbortError') {
          console.warn(`Direct OpenRouter call timed out for model ${model}`);
        } else {
  ...
  const initExistingUserSession = (userData, settingsData, serverStreak = null) => {
    const cleanEmail = (userData?.email && typeof userData.email === 'string' && userData.email.includes('@')) ? userData.email.trim().toLowerCase() : '';
    const userIdHint = userData?.id || userData?.user_id || cleanEmail || null;
    const savedAvatar = cleanEmail ? localStorage.getItem(`mindease_user_avatar_${cleanEmail}`) : null;
    ...
    // Pre-load cached checkins synchronously so dailyStreak is correct immediately
    // on login, without waiting for the async Flask fetch to complete.
    // refreshDailyCheckins() below will overwrite with authoritative server data.
    let cachedCheckins = [];
    if (userIdHint) {
      try {
        const cacheKey = `mindease_daily_checkins_${String(userIdHint).replace(/[^a-zA-Z0-9_@.-]/g, '_')}`;
        const raw = localStorage.getItem(cacheKey);
        if (raw) {
          const parsed = JSON.parse(raw);
          if (Array.isArray(parsed)) cachedCheckins = parsed;
        }
      } catch (e) { }
    }

    // Pre-populate dailyCheckins with the cache immediately (no flicker to 0)
    setDailyCheckins(cachedCheckins);
    setUserEnrollments([]);
    ...
    setData((prev) => ({
      ...
      profile: {
        ...
        // Use server-provided streak first (most reliable — from DB).
        // Fall back to local cache calculation if server didn't return it.
        streak: (typeof serverStreak === 'number') ? serverStreak : calculateStreak(cachedCheckins),
    ...
    if (cleanEmail) {
      ...
      refreshUserChatHistory(cleanEmail);
      refreshUserCommunityPosts(cleanEmail);
      // Fetch fresh check-ins from Flask DB — updates dailyStreak once resolved
      refreshDailyCheckins(userIdHint);
