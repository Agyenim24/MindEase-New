...
function NotificationPanel({ open, onClose }) {
  const { notifications, unreadCount, readIds, markRead, markAllRead, dismissNotification, desktopPermission, requestDesktopNotifications } =
    useNotifications();
  ...
  const renderItem = (n) => {
    ...
    const tagText = n.type === 'system' ? 'Welcome' : n.type === 'program' ? 'Program' : n.tag || 'Community';
    ...
    <>
      ...
      >
        ...
        {desktopPermission === 'default' && (
          <div className="bg-primary/10 px-4 py-2.5 flex items-center justify-between gap-2 border-b border-primary/10">
            <div className="flex items-center gap-2 text-primary">
              <span className="material-symbols-outlined text-base">notifications_active</span>
              <span className="text-[11px] font-bold">Enable Desktop Alerts</span>
            </div>
            <button
              onClick={requestDesktopNotifications}
              className="px-3 py-1 bg-primary text-white text-[10px] font-bold rounded-full hover:opacity-90 transition shadow-sm"
            >
              Enable
            </button>
          </div>
          ...
          {programs.length > 0 && (
            <section className="space-y-2">
              <div className="flex items-center gap-2 px-1">
                ...
                <span className="text-xs font-bold text-on-surface uppercase tracking-wider">
                  Program Reminders ({programs.length})
                </span>
              </div>
              <div className="space-y-2">
