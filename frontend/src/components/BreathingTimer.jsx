...
function BreathingTimer({ exercise, onClose }) {
  if (!exercise || !exercise.phases || !Array.isArray(exercise.phases) || exercise.phases.length === 0) {
    return (
      <div className="p-6 text-center text-on-surface-variant">
        <p className="font-bold">Breathing exercise unavailable.</p>
        {onClose && (
          <button onClick={onClose} className="mt-3 px-4 py-2 bg-primary text-white rounded-xl text-xs font-bold">
            Close
          </button>
        )}
      </div>
  ...
  const phases = exercise.phases.map(ph => ({
    ...ph,
    duration: ph.duration || ph.seconds || 4
  }));
  ...
  const progress = currentPhase.duration ? (1 - secondsLeft / currentPhase.duration) : 0;

  const getScale = () => {
    const label = currentPhase.label ? currentPhase.label.toLowerCase() : "";
    if (label.includes("inhale")) return 0.6 + progress * 0.4;
    if (label.includes("exhale")) return 1 - progress * 0.4;
    return 1;
  };
