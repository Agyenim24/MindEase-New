  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-xl md:max-w-2xl bg-surface rounded-3xl p-6 md:p-8 shadow-2xl animate-[slideUp_0.3s_ease-out] flex flex-col items-center text-center z-10 border border-outline-variant/20">
        ...
        <div className="grid grid-cols-5 gap-1.5 sm:gap-4 w-full mb-6">
          {MOODS.map((m) => (
            <button
              key={m.id}
              onClick={() => onSelect(m.id)}
              className="flex flex-col items-center justify-center gap-1 p-2 sm:p-4 rounded-2xl bg-surface-container-low border border-outline-variant/10 hover:border-primary hover:bg-primary/10 hover:scale-105 transition-all active:scale-95 group w-full"
            >
              <span className="text-2xl sm:text-4xl group-hover:scale-110 transition-transform">{m.emoji}</span>
              <span className="text-[10px] sm:text-xs font-bold text-on-surface-variant group-hover:text-primary transition-colors truncate max-w-full">{m.label}</span>
            </button>
          ))}
        </div>
...
function ProgramDetail({ program, onClose, onStart, onEnroll, toggleModuleCompletion, logActivityMood, setActivityReminder }) {
  ...
  const isEnrolled = program.enrolled;
  const isCompleted = program.status === 'completed' || (total > 0 && completed === total);
      ...
      <div className="relative w-full max-w-4xl bg-surface max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl flex flex-col">
        ...
        <div className="flex-1 p-5 space-y-5">
          ...
          {isEnrolled && (
            <div className="bg-surface-container-low rounded-2xl p-4 space-y-3">
              <div className="flex justify-between text-sm font-bold">
                <span className="text-on-surface">Your Progress</span>
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-sm text-on-surface">About this program</h4>
          ...
          <div className="bg-surface-container-low rounded-2xl p-4 space-y-2 border border-outline-variant/10">
            ...
            <ul className="space-y-1.5 text-xs text-on-surface-variant">
              <li className="flex items-center gap-2"><span className="text-emerald-500 font-bold">✓</span> Identify common stress & anxiety triggers</li>
              <li className="flex items-center gap-2"><span className="text-emerald-500 font-bold">✓</span> Understand your emotional and nervous system patterns</li>
              <li className="flex items-center gap-2"><span className="text-emerald-500 font-bold">✓</span> Learn practical grounding & relaxation techniques</li>
              <li className="flex items-center gap-2"><span className="text-emerald-500 font-bold">✓</span> Build healthier daily wellness habits</li>
              <li className="flex items-center gap-2"><span className="text-emerald-500 font-bold">✓</span> Develop a personalized stress-management routine</li>
            </ul>
          </div>
          ...
          {!isEnrolled ? (
            ...
            <div className="w-full py-3.5 bg-emerald-500/10 text-emerald-600 rounded-2xl font-bold flex items-center justify-center gap-2 border border-emerald-500/20">
              ...
              🎉 Program Completed! Great work!
            </div>
          ) : (
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs font-bold text-primary px-1">
                <span>Enrolled (Day {program.currentActivity || 1} of {total})</span>
...
// ─── Activity Modal ───────────────────────────────────────────────────────────
function ActivityModal({ module, onClose, onCompleteActivity }) {
  ...
  const exercInstr = (module.id && EXERCISE_INSTRUCTIONS[module.id]) || EXERCISE_INSTRUCTIONS.default;
  ...
  const articleText = article ? (article.content || article.body || article.summary || "") : "";
  const safeMindText = (script && script[mindIdx] && script[mindIdx].text) || "Take a deep breath and focus on the present moment.";
      ...
      <div className="relative w-full max-w-4xl bg-surface rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {module.type === "breathing" && (
            <div>
              <p className="text-on-surface-variant text-sm mb-4">{exercise?.description || "Follow the guided breathing timer to calm your mind."}</p>
              <BreathingTimer exercise={exercise || BREATHING_EXERCISES.b1} onClose={onClose} />
            </div>
          )}
          ...
          {module.type === "mindfulness" && (
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between text-sm text-on-surface-variant">
                ...
                <span>{mindIdx + 1} / {script.length || 1}</span>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="prose prose-sm md:prose-base text-on-surface-variant leading-relaxed space-y-4 text-sm md:text-base">
                    {articleText ? (
                      ...
                      <p>{article.summary || "Read through this article to gain valuable mental wellness insights."}</p>
                    )}
                  </div>
                  {article.disclaimer && <p className="text-xs text-on-surface-variant italic">{article.disclaimer}</p>}
                </div>
              ) : (
          ...
          {/* Exercise */}
          {module.type === "exercise" && exercInstr && (
            <div className="space-y-4">
        ...
        {/* Footer Action Bar */}
        <div className="p-4 bg-surface-container-low border-t border-outline-variant/10 flex items-center justify-between gap-3 shrink-0">
          <button onClick={onClose} className="px-4 py-2 text-xs font-bold text-on-surface-variant hover:text-primary">
            Close
          </button>
          <button
            onClick={() => onCompleteActivity && onCompleteActivity(module)}
            className="px-6 py-2.5 bg-primary text-white rounded-xl text-xs font-bold hover:opacity-90 active:scale-95 transition shadow-md shadow-primary/20 flex items-center gap-1.5"
          >
...
function Programs() {
  ...
  const {
    ...
    enrollUserProgram, completeUserActivity, refreshEnrollments
  } = useData();
  ...
  const handleEnroll = async (programId) => {
    const res = await enrollUserProgram(programId);
    if (res?.alreadyEnrolled) {
      showToast("You're already enrolled in this program.");
    } else if (res?.success) {
  ...
  const handleCompleteActivity = async (module) => {
    if (!module) return;
    const prog = programs.find((p) => p.modules && p.modules.some((m) => String(m.id) === String(module.id)));
    if (prog) {
      const totalMods = prog.modules.length;
      const res = await completeUserActivity(prog.id, module.id, totalMods);
      toggleModuleCompletion(prog.id, module.id);
      if (res?.isCompleted) {
        showToast("🎉 Program Completed! Congratulations!");
      } else {
        showToast("Activity Completed! Great work! 👍");
      }
      handleCloseModal();
    }
  };
  ...
  const handleStart = (module) => {
    if (!module) return;
    const prog = programs.find((p) => p.modules && p.modules.some((m) => String(m.id) === String(module.id)));
    const progId = prog ? prog.id : 1;
    setPendingModule(module);
    setMoodPicker({ programId: progId, moduleId: module.id, phase: "before" });
  };
  ...
  const handleMoodSelect = (mood) => {
    if (moodPicker) {
      ...
      if (moodPicker.phase === "before") {
        setMoodPicker(null);
        if (targetModule) {
          setActiveModal({ module: targetModule });
        }
      } else {
        setMoodPicker(null);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
      {/* Toast Overlay */}
      {toastMsg && (
        <div className="fixed top-24 right-6 z-50 bg-surface-container-highest text-on-surface border border-outline-variant/30 px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 animate-[slideDown_0.3s_ease-out]">
        ...
        <div className="max-w-[1200px] w-full mx-auto px-margin-mobile md:px-margin-desktop py-lg space-y-xl flex-grow">
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
            {[
              ...
              { icon: "local_fire_department", label: "Day Streak", value: `${profile?.streak || 0}d`, color: "text-amber-500" },
            ].map((s) => (
          ...
          {assessmentProfile && recommendedProg && !recommendedProg.enrolled && (
            <section className="relative overflow-hidden bg-gradient-to-r from-primary to-secondary rounded-3xl p-6 md:p-8 shadow-xl">
              ...
              <div className="relative flex flex-col md:flex-row items-start md:items-center gap-6">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => handleEnroll(recommendedProg.id)}
          ...
          {!assessmentProfile && (
            <section className="bg-gradient-to-r from-primary-container/30 to-secondary-container/20 p-6 md:p-8 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6 border border-outline-variant/20 mt-5">
          ...
          <section>
            <div className="flex justify-between items-end mb-5 mt-5 ">
              <div>
                <h2 className="font-headline-lg text-headline-lg text-on-surface font-bold">My Programs</h2>
            ...
            {activePrograms.length > 0 ? (
                    ...
                    <div key={prog.id} className="bg-surface-container-lowest rounded-[28px] p-6 md:p-8 shadow-sm border border-outline-variant/20 flex flex-col md:flex-row gap-6 items-center hover:shadow-md transition-shadow">
                      ...
                      <div className="flex-1 space-y-3 w-full">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isDone && (
                            <span className="px-2.5 py-0.5 bg-emerald-500/10 text-emerald-600 rounded-full text-xs font-bold border border-emerald-500/20">
                              ✓ Completed
                            </span>
                          )}
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between text-xs font-bold">
                            <span className="text-on-surface-variant">
                              Day {prog.currentActivity || 1} of {prog.modules.length} ({prog.modules.filter((m) => m.completed).length} / {prog.modules.length} activities)
                            </span>
                        ...
                        <div className="flex gap-3 pt-1">
                          ...
                          >
                            <span className="material-symbols-outlined text-base">{isDone ? "check_circle" : "play_circle"}</span>
                            {isDone ? "Review Program" : `Continue Day ${prog.currentActivity || 1}`}
                          </button>
                        </div>
                      </div>
                    </div>
              ...
              <div className="p-10 text-center bg-surface-container-lowest rounded-3xl border border-outline-variant/20 space-y-3">
                <span className="material-symbols-outlined text-5xl text-outline mb-1" style={{ fontVariationSettings: "'FILL' 1" }}>school</span>
                <p className="text-on-surface font-bold text-lg">You haven't enrolled in a program yet.</p>
                <p className="text-on-surface-variant text-sm max-w-md mx-auto">Explore the available programs below and click "Enroll in Program" to start your wellness journey.</p>
                <button
                  onClick={() => {
                    const el = document.getElementById('explore-programs');
                    if (el) el.scrollIntoView({ behavior: 'smooth' });
                  }}
                  className="px-6 py-2.5 bg-primary text-white rounded-full text-xs font-bold hover:opacity-90 transition shadow-md shadow-primary/20 inline-flex items-center gap-1.5 mt-2"
                >
          ...
          <section id="explore-programs" className="space-y-md">
            <h2 className="font-headline-lg text-headline-lg text-on-surface font-bold mt-5">Explore All Programs</h2>
              ...
              {filteredPrograms.map((prog) => (
                <div key={prog.id} className="group bg-surface-container-lowest rounded-3xl overflow-hidden border border-outline-variant/30 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
                  ...
                  <div className="p-5 flex flex-col flex-1 gap-3">
                    ...
                    <button
                      onClick={() => prog.enrolled ? setDetailProgram(prog) : handleEnroll(prog.id)}
                      className={`w-full py-3 rounded-2xl font-bold transition-all text-sm ${prog.enrolled
                        ? "bg-primary text-white hover:opacity-90"
                        : "bg-primary/10 text-primary hover:bg-primary hover:text-white"
                        }`}
                    >
                      {prog.enrolled ? (prog.status === 'completed' ? "✓ Completed" : `Continue Day ${prog.currentActivity || 1} →`) : "Enroll in Program"}
                    </button>
                  </div>
                </div>
              ))}
