...
import { apiGetResources } from '../services/api';
...
function Resources() {
  ...
  const [displayVideoSessions, setDisplayVideoSessions] = useState(videoSessions);
  ...
  useEffect(() => {
    async function loadDbVideos() {
      ...
      if (res && res.resources) {
        const vids = res.resources
          .filter((r) => r.type === 'video')
          .map((r) => ({
            id: r.id,
            title: r.title,
            guide: r.guide || r.content || 'Guided Video Practice',
            duration: r.duration || r.read_time || '15:00',
            thumbnail: r.image_url || r.bg_url,
            videoUrl: r.video_url || '/assets/videos/streessRelief.mp4',
            description: r.description,
          }));
        if (vids.length > 0) {
          setDisplayVideoSessions(vids);
        }
      }
    }
    loadDbVideos();
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {BREATHING_EXERCISES.map((ex) => (
                <div
                  ...
                  className={`p-5 rounded-3xl bg-surface-container-lowest border border-outline-variant/30 hover:border-primary/40 hover:shadow-sm transition-all duration-200 cursor-pointer flex flex-col justify-between group space-y-4 relative overflow-hidden`}
                >
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div
                  ...
                  className="md:col-span-2 group relative h-[380px] rounded-3xl overflow-hidden cursor-pointer shadow-sm hover:shadow-md transition-all duration-300"
                >
                  <div
                ...
                <div className="flex flex-col gap-6">
                  {sideArticles.map((card) => (
                    <div
                      ...
                      className="group h-[178px] rounded-3xl bg-surface-container-lowest border border-outline-variant/30 p-6 flex flex-col justify-between cursor-pointer hover:border-primary/40 hover:shadow-sm transition-all duration-200 relative"
                    >
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        <span className="material-symbols-outlined text-primary group-hover:translate-x-0.5 transition-transform duration-200 text-lg">
                          arrow_forward
                        </span>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {remainingArticles.map((item) => (
                  <div
                    ...
                    className="flex flex-col justify-between p-5 rounded-3xl bg-surface-container-lowest border border-outline-variant/20 shadow-sm hover:shadow-md hover:border-primary/30 transition-all duration-200 cursor-pointer group space-y-4"
                  >
                    <div className="space-y-3">
                      <div className="h-44 rounded-2xl overflow-hidden relative">
                        ...
                        <button
                          ...
                          className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/40 backdrop-blur text-white flex items-center justify-center hover:bg-black/60 transition-colors"
                        >
                          <span
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-outline-variant/10 text-xs">
                      ...
                      <span className="text-primary font-bold group-hover:translate-x-0.5 transition-transform duration-200 flex items-center gap-1">
          ...
          <section className="bg-surface-container-low -mx-margin-mobile md:-mx-margin-desktop px-margin-mobile md:px-margin-desktop py-12">
            <div className="max-w-[1440px] mx-auto">
              ...
              <div className="flex gap-6 overflow-x-auto hide-scrollbar pb-2">
                {displayVideoSessions.map((vid) => (
                  <div
                    key={vid.id}
                    onClick={() => setActiveVideo(vid)}
                    className="min-w-[280px] md:min-w-[340px] flex-shrink-0 group cursor-pointer"
                  >
                    <div className="relative aspect-video rounded-2xl overflow-hidden mb-3 shadow-sm group-hover:shadow-md transition-shadow duration-200 bg-black">
                      <img
                        src={vid.thumbnail}
                        alt={vid.title}
                        className="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-300 ease-out"
                      ...
                      <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-colors duration-200 flex items-center justify-center">
                        <div className="w-14 h-14 rounded-full bg-white/90 backdrop-blur flex items-center justify-center shadow-xl transform group-hover:scale-105 transition-transform duration-200">
                          <span
